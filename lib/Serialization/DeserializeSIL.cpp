...
SILDeserializer::readSILFunctionChecked(DeclID FID, SILFunction *existingFn,
  ...
  // SWIFT_ENABLE_TENSORFLOW
  // If we use an autogenerated location as before, it causes problems if we
  // inline deserialized functions in MandatoryInlining.  (See llvm::unreachable
  // MandatoryInlinedLocation::getMandatoryInlinedLocation in SILLocation.cpp).
  // We are creating something meaningful for the time being.
  SILLocation loc = RegularLocation::getModuleLocation();
